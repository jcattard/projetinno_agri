...
def lectureControle(controlData):
    ...
    for k in range(1,nbr_lines):
        # print(controlData[k])
        DOY[k-1] = float(controlData[k][1])-299
        IRRAD[k-1] = float(controlData[k][2])
        T2M[k-1] = float(controlData[k][4])
        T2MN[k-1] = float(controlData[k][5])
        T2MX[k-1] = float(controlData[k][6])
        HUM[k-1] = float(controlData[k][7])/100
...
if __name__ == "__main__" :
    ...
    par=dict()
    with open(args.parameters_file,'r') as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
            par[row[0]] = float(row[1])
    exp=dict()
    with open(args.experience_file,'r') as csvfile:
        reader=csv.reader(csvfile)
        for row in reader:
            exp[row[0]] = float(row[1])
    ...
    with open(args.control_file,'r') as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
